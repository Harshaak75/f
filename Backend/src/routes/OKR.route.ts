  ...
  try {
      ...
      include: {
        ...
        keyResults: {
          include: {
            ...
            owner: {
              select: { name: true, employeeProfile: { select: { firstName: true, lastName: true } } },
            },
          },
        },
      },
    ...
    const formattedObjectives = objectives.map(obj => ({
      ...
      keyResults: obj.keyResults.map(kr => ({
        id: kr.id,
        keyResult: kr.title,
        owner: kr.owner.employeeProfile ? `${kr.owner.employeeProfile.firstName} ${kr.owner.employeeProfile.lastName}` : kr.owner.name,
...
router.post('/objective', protect, async (req, res) => {
    ...
    if (!title || !quarter || !year || !ownerId) {
        return res.status(400).json({ message: 'Title, quarter, year, and ownerId are required.' });
    }
...
router.post('/keyresult', protect, async (req, res) => {
    ...
    if (!title || !ownerId || !objectiveId) {
        return res.status(400).json({ message: 'Title, ownerId, and objectiveId are required.' });
    }
...
router.put('/keyresult/:krId', protect, async (req, res) => {
    ...
    if (progress === undefined || !status) {
        return res.status(400).json({ message: 'Progress and status are required.' });
    }
    ...
    try {
        ...
        if (role !== Role.ADMIN && keyResult.ownerId !== userId) {
            return res.status(403).json({ message: 'Forbidden: You are not the owner of this key result.' });
        }
